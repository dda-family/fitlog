 ...
 */
const Backup = {
  STORES: ["settings", "exercises", "routineTemplates", "routineGuides", "workoutSessions", "guideHistory", "userMuscleMap"],

  async exportAll() {
    const DB = window.FitlogDB.DB; await DB.open();
    const data = {
      app: "fitlog", schemaVersion: 2, exportedAt: new Date().toISOString(),
      ...
      userMuscleMap: await DB.getAll("userMuscleMap"),
    };
  ...
  validate(obj) {
    ...
    // v1(userMuscleMap 없음)·v2 모두 허용. v1 백업 복원 시 userMuscleMap은 빈 값으로 둔다(하위호환).
    if (obj.schemaVersion !== 1 && obj.schemaVersion !== 2) return { ok: false, reason: "지원하지 않는 스키마 버전입니다." };
  ...
  async importAll(file) {
    ...
    try {
      ...
      for (const x of (obj.userMuscleMap || [])) await DB.put("userMuscleMap", x);
      return { ok: true };
    } catch (e) {
