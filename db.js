...
// v1 → v2: userMuscleMap 스토어 추가(가산적). 기존 스토어·데이터는 그대로 유지.
// onupgradeneeded는 STORES를 순회하며 "없는 스토어만" 생성하므로 기존 데이터 보존 + 롤백 안전.
const DB_VERSION = 2;
const STORES = {
  settings:         { keyPath: "id" },
  ...
  // 사용자 지정 근육 매핑. key=기존 앱 운동ID. 값 {id, catalogRef, mapping:{primary,secondary,minor}, source, version, updatedAt}
  // 기존 운동/가이드/기록과 완전히 분리된 별도 스토어라, 여기에 무엇을 넣어도 기존 데이터에 영향 없음.
  userMuscleMap:    { keyPath: "id" },
};
...
const DB = {
  ...
  // ── 사용자 근육 매핑 (userMuscleMap 스토어) ─────────────────────────
  // 히트맵 compute가 쓰는 형태: { [exId]: { mapping:{primary,secondary,minor}, ... } }
  async userMuscleMapAll() {
    await this.open();
    const rows = await this.getAll("userMuscleMap");
    const out = {};
    for (const r of rows || []) if (r && r.id) out[r.id] = r;
    return out;
  },
  async setUserMuscleMap(exId, entry) {
    await this.open();
    const row = Object.assign({ id: exId, version: 1, updatedAt: new Date().toISOString() }, entry, { id: exId });
    await this.put("userMuscleMap", row);
    return row;
  },
  async deleteUserMuscleMap(exId) {
    await this.open();
    return this.delete("userMuscleMap", exId);
  },
