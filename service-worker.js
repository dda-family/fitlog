 ...
 */
const CACHE_VERSION = "fitlog-v27";
...
// 브라우저 HTTP 캐시(GitHub Pages max-age=600)에 남은 이전 버전 파일이 새 캐시에 섞이지 않도록 서버에서 새로 받는다.
const fresh = (u) => new Request(u, { cache: "reload" });
self.addEventListener("install", (e) => {
  e.waitUntil(caches.open(CACHE_VERSION).then(async (c) => {
    await c.addAll(SHELL.map(fresh));                       // 필수 셸(원자적)
    await Promise.all(ANATOMY.map((u) => c.add(fresh(u)).catch(() => {})));   // SVG는 best-effort
    await Promise.all(THREE3D.map((u) => c.add(fresh(u)).catch(() => {})));   // 3D 자산은 best-effort(실패 무시)
  }));
  self.skipWaiting();
});
...
self.addEventListener("fetch", (event) => {
  ...
  // 3D 기술 시제품(공유 인체·클립·장비): 시험 화면에서 요청된 것만 저장, 프리캐시 안 함.
  const isExerciseAnim = url.pathname.includes("/assets/exercises/") || url.pathname.includes("/assets/prototype/");
  if (isAsset) {
    ...
    // 캐시 미스 시 HTTP 캐시 대신 서버 재검증(no-cache: 같으면 304) → 같은 파일명의 구버전 GLB·manifest 혼입 방지
    event.respondWith(caches.match(event.request).then((cached) => cached || fetch(new Request(event.request, { cache: "no-cache" })).then((res) => { const clone = res.clone(); if (res.ok) caches.open(CACHE_VERSION).then((c) => c.put(event.request, clone)); return res; })));
  } else {
